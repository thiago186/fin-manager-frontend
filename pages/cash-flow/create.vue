                    ...
                    <div v-else class="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-48 overflow-y-auto p-2 border rounded-md bg-white">
                      ...
                      >
                        <Checkbox
                          ...
                          :checked="group.category_ids.includes(category.id)"
                          @update:checked="(checked: boolean) => toggleCategory(groupIndex, category.id, checked)"
                          @click.stop
                        />
                        <Label
                          :for="`group-${groupIndex}-cat-${category.id}`"
                          class="text-sm font-normal cursor-pointer flex-1"
                          @click.stop="toggleCategory(groupIndex, category.id, !group.category_ids.includes(category.id))"
                        >
                          {{ category.name }}
                          <span
                            :class="[
...
const toggleCategory = (groupIndex: number, categoryId: number, checked: boolean) => {
  ...
  const newCategoryIds = [...group.category_ids]
  ...
  if (checked) {
    ...
    const idx = newCategoryIds.indexOf(categoryId)
    if (idx > -1) {
